...
describe('carts.test', () => {
    ...
    beforeAll(() => {
        ...
        // -------------- DELETE BEFORE MERGE ------------------------------------------
        mongoose.connect('mongodb://localhost:27017/?readPreference=primary&appname=MongoDB%20Compass&ssl=false', mongooseOptions);
        // -------------- DELETE BEFORE MERGE ------------------------------------------
    });
    ...
    describe('PUT /api/v1/cart/', () => {
        ...
        beforeAll( async() => {
            ...
            // -------------- DELETE BEFORE MERGE ------------------------------------------
            db.carts.insert( {amount: 5, userId: userId, productId: productId}, function ( err, data ) {
    			if( err ){
    				res.status( 500 ).json({
    					success: false,
