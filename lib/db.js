...
const products = {
    ...
    async findOne(id, callback) {
        ...
        let pr = await Product.findOne({ "_id": id });
        callback(pr);
    },

    //to delete a product no available
    async deleteOne(id, callback) {
        let dl = await Product.findOneAndDelete({ "_id": id }, null);
        callback(dl);
    },

    //to update the number of availeble products
    async updateOne_amount(id, amount, callback) {
        ...
        let up = await Product.findOneAndUpdate({ "_id": id }, {amount: amount}, null);
        callback(up);
    }
};
...
const orders = {
    ...
    async insert(newOrder, callback) {
        ...
        await insertOrder.save(function (err, data) {
            callback(data);
        });
    },

    async deleteOne(id, callback) {
        let dl = await Order.findOneAndDelete({ "_id": id }, null);
        callback(dl);
    }
};
...
const users = {
    async findOne(params, callback) {
        ...
        let us = await User.findOne(params);
        callback(us);
    }
};
