...
const products = {
    ...
    findByUserId(userId, callback) {
        Product.find({"userId" : userId}).exec( (err, data) => {
            callback(err, data);
        });
    },
    ...
    search(params, callback) {
        Product.find(params).limit(3).exec( (err, data) => {
            callback(err, data);
        });
    }
...
const reviews = {
    ...
    // to find a review by params and update it
    findOneAndUpdate(params, set, callback) {
        Review.findOneAndUpdate(params, set, {new: true}, (err, data) => {
            callback(err, data);
        });
    }
}
