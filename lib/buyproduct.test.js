...
describe('buyproduct.test', () => {
    ...
    beforeAll(async() => {
        ...
        var newProductId = db.products.insert({name: "producttest",
                                       image: "testjpg",
                                       description: "test",
                                       price: 1234,
                                       amount: 1234,
    ...
    test('works with put, amount > 1', async() => {
    ...
    test('works with put, amount == 1', async () => {
        var response = await fetch(url, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json', 'x-access-token': token },
            body: JSON.stringify({ product_id: product_id_amount_1, user_id: user_id, address: address, numCard: numCard, expCard: expCard }),
        });
