		...
		jwt.verify(token, key, function (err, decoded) {
			if (err) {
				...
				if ( id_user.match(/^[0-9a-fA-F]{24}$/) ) {
					...
					return res.status(403).json({ success: false, error: 'User id bad format.' })
                }
...
router.put('', (req, res) => { 
	...
	if (!expCard || typeof expCard != 'string') {
		res.status(400).json({ success: false, field:"expCard", error:'The field expCard must be a non-empty string, in string format' });
		return;
	} else {
		let d = new Date();
