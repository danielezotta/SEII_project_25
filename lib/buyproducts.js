		...
		jwt.verify(token, key, function (err, decoded) {
			if (err) {
				...
				if ( id_user.match(/^[0-9a-fA-F]{24}$/) ) {
					...
					return res.status(404).json({ success: false, error: 'User id bad format.' })
                }
...
router.put('', (req, res) => { 
	...
	if ( !id_product || typeof id_product != 'string' || !id_product.match(/^[0-9a-fA-F]{24}$/) ) {
		res.status(400).json({ success: false, error: 'The field id_product must be a non-empty string, in string format' });
        return;
	}

	if (!address || typeof address != 'string') {
		res.status(400).json({ success: false, error: 'The field address must be a non-empty string, in string format' });
		return;
	}

	if (!numCard || isNaN(numCard)) {
		res.status(400).json({ success: false, error: 'The field numCard must be a non-empty string, in number format' });
		return;
	}

	if (!expCard || typeof expCard != 'string') {
		res.status(400).json({ success: false, error: 'The field expCard must be a non-empty string, in string format' });
		return;
	} else {
		let d = new Date();
		let exp = expCard.split("/");
		if (parseInt(exp[1]) < d.getFullYear()) {
			res.status(400).json({ success: false, error: 'The current year can\'t be greater than expiry year' });
			return;
		} else {
			if (parseInt(exp[1]) == d.getFullYear() && parseInt(exp[0]) < d.getMonth()) {
				res.status(400).json({ success: false, error: 'The current month can\'t be greater than expiry month' });
				return;
			}
        }
    }
